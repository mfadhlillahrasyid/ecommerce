...
import { h } from "../utils/hashLink.js";
  ...
  return `
        <section class="space-y-6">
          ...
          ${categories
            .map(
              (c) =>
                `<a class="px-3 py-1.5 rounded-xl text-sm bg-white/5 border border-white/10 hover:bg-white/10" href="${h(
                  "/category/" + c.slug
                )}">${c.name} <span class="text-white/40">(${
                  c.count
                })</span></a>`
            )
            .join("")}
              ...
              .map(
                (p) => `
                  <article class="group bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                    <a a class="block" href="${h("/product/" + p.slug)}">
  ...
  function paint(list) {
    grid.innerHTML = list
      .map(
        (p) => `
<article class="group bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
<a class="block" href="${h("/product/" + p.slug)}">
<div class="aspect-square overflow-hidden bg-white/5">
<img src="${p.image}" alt="${
          p.title
        }" class="w-full h-full object-cover group-hover:scale-[1.02] transition" loading="lazy"/>
