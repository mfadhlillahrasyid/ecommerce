...
import { h } from "../utils/hashLink.js"; // <-- biar href pakai "#/..."
...
<div class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl p-3">
  ...
  <div class="flex-1 min-w-0">
    <div class="flex items-center justify-between gap-2">
      <a href="${h('/product/'+p.slug)}" class="font-semibold line-clamp-1">${p.title}</a>
  ...
  const msgLines = items
    .map((it) => {
      const p = products.find((x) => String(x.id) === String(it.id));
      return p ? `• ${p.title} — ${fmtIDR(p.price)} x ${it.qty}` : null;
    })
    .filter(Boolean);
  const waMsg = encodeURIComponent(
    `Halo admin ${shopName}, saya ingin checkout:\n\n${msgLines.join("\n")}\n\nSubtotal: ${fmtIDR(subtotal)}\nMetode: WhatsApp`
  );
...
<section class="space-y-6">
  ...
  ${items.length === 0
    ? `<div class='text-white/60'>Keranjang kosong. <a href='${h("/")}' class='underline'>Belanja dulu</a>.</div>`
    : `
  <div class="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4">
    ...
    <div class="flex flex-wrap gap-3">
      <a id="checkoutWA" href="https://wa.me/${waPhone}?text=${waMsg}" target="_blank" rel="noopener"
         class="inline-flex items-center justify-center rounded-xl px-4 py-2 border border-emerald-500/30 bg-emerald-500/15 text-emerald-300 hover:bg-emerald-500/20 active:scale-[.98] transition">
        Checkout via WhatsApp
      </a>
      <a href="${h('/')}" class="inline-flex items-center justify-center rounded-xl px-4 py-2 border border-white/10 hover:bg-white/10">Lanjut Belanja</a>
    </div>
  </div>`}
</section>`;
}

// soft rerender tanpa reload
...
export function bindCart() {
  ...
  wrap.addEventListener("click", (e) => {
    ...
    if (dec) {
      ...
      const input = wrap.querySelector(`[data-qty="${CSS.escape(id)}"]`);
      const newQty = toInt(input?.value) - 1;
      updateQty(id, newQty);
      softRefresh();
    }
    if (inc) {
  ...
  wrap.addEventListener("change", (e) => {
    ...
    updateQty(id, toInt(input.value));
    softRefresh();
  });
}
