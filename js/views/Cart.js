...
<img class="w-16 h-16 rounded-lg object-cover" src="${p.image}" alt="${
      p.title
    }"/>
<div class="flex-1 min-w-0">
<div class="flex items-center justify-between gap-2">
<a href="/product/${p.slug}" class="font-semibold line-clamp-1">${p.title}</a>
<button data-remove="${
      p.id
    }" class="text-xs text-white/60 hover:text-white">Hapus</button>
</div>
...
<input data-qty="${p.id}" type="number" min="1" value="${
      it.qty
    }" class="w-12 px-0 py-1 text-center bg-transparent focus:outline-none" />
...
${
  items.length === 0
    ? `<div class='text-white/60'>Keranjang kosong. <a href='/' class='underline'>Belanja dulu</a>.</div>`
    : ""
}
${
  items.length > 0
...
function softRefresh() {
  // trigger router render lagi tanpa reload network
...
export function bindCart() {
  ...
  wrap.addEventListener("click", (e) => {
    ...
    if (rm) {
      ...
      removeItem(id);
      softRefresh(); // ⬅️ was: location.reload()
    }
  });
  ...
  wrap.addEventListener("change", (e) => {
    ...
    if (qty) {
      ...
      updateQty(id, Math.max(1, +qty.value || 1));
      softRefresh(); // ⬅️ was: location.reload()
    }
  });
}